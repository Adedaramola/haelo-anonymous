        ...
        <div class="w-full max-w-sm px-6 py-5 bg-white border border-gray-100 rounded">
            ...
            <form action="{{ route('user', $user) }}" method="post">
                ...
                <textarea x-data="{
                 ...
                 x-init="resize" x-on:input="resize" name="message" class="block w-full focus:outline-none resize-none" placeholder="you're so awesome i forget to tell you everyday." required></textarea>
                <button type="submit" class="block w-full px-5 py-3 text-yellow-200 font-medium bg-indigo-900 rounded-3xl">Send</button>
            </form>
        </div>
    </div>
@endsection
